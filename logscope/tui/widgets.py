...
def render_cluster_table(templates: Sequence[Template], top_n: int = 10) -> Table:
    """Render the top templates as a ranked count + bar + template table.

    Templates fold onto multiple lines rather than truncating, so the full
    pattern is always readable even in the narrow side panel.
    """
    ...
    table.add_column("", width=4, no_wrap=True)  # proportional bar
    table.add_column("template", ratio=1, overflow="fold")

    top = list(templates[:top_n])
    max_count = max((t.count for t in top), default=1) or 1
    for template in top:
        bar_len = round(4 * template.count / max_count)
